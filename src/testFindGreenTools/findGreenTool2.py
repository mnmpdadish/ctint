...
def symmetrizeElementOneByOne(symmetryRuleList,greenFunction,gL,n_sites,verbose):
   ...
   for element in symmetryRuleList:
      if gL==len(element.symmetry):
         sym = []
         for ii in range(0,len(element.symmetry)): sym.append(int(element.symmetry[ii]))
         
      elif gL==2*len(element.symmetry):
         sym = []
         for ii in range(0,len(element.symmetry)): sym.append(int(element.symmetry[ii]))
         for ii in range(0,len(element.symmetry)): sym.append(int(element.symmetry[ii])+len(element.symmetry))   
         
      else:
         print 'error: incoherent dimensions'; exit()
      ...
      for i in range(0,gL):
         for j in range(0,gL):
            ...
            nn = 0
            isFirst = False
            if (permutation_i==i) and (permutation_j==j):
               isFirst = True  # important for generator that corresponds to identity (but with sign change)
            ...
            while ((permutation_i != i) or (permutation_j != j)) or isFirst: #while didn't loop on orbit
               ...
               if verbose > 7:
                  print nn,i,j,permutation_i, permutation_j, greenFunction[permutation_j,permutation_i].string(), greenFunction[j,i].string()
               factor = 1.0
               
               isConj=False
               if ((i<n_sites) and (j<n_sites)) or ((i>=n_sites) and (j>=n_sites)):
                  if element.factors[0]=='+':
                     factor = 1.0
                  else:
                     factor = (-1.0)**nn
               else:
                  if element.factors[1]=='+':
                     factor = 1.0
                  else:
                     factor = (-1.0)**nn
               ...
               if (greenFunction[j,i].string() is not '0') and (greenFunction[permutation_j,permutation_i].string() is not '0'):
                  if not(greenFunction[permutation_j,permutation_i].isEqual(greenFunction[j,i],factor,isConj)): 
                     if verbose > 7:
                        ...
                        print isConj
                     assert len(greenFunction[j,i].dic.keys())==1, 'oups'
                     assert len(greenFunction[permutation_j,permutation_i].dic.keys())==1, 'oups'
                     name0 = (greenFunction[j,i].dic.keys())[0]
                     name1 = (greenFunction[permutation_j,permutation_i].dic.keys())[0]
                     if verbose > 7: print printGreenFunctionToString(greenFunction,gL)
